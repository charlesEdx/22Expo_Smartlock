...
int jrk_set_target(HANDLE port, uint16_t target)
{
	...
	{
		uint8_t cmd[2];
		cmd[0] = 0x5a; cmd[1]=0x33;
		write_port(port, cmd, sizeof(cmd));
	}
	return 0;
#else
...
{
	if (argc < 3) {
		printf("\nUsage: %s comport ledcmd \n", argv[0]);
		printf("\n\tcomport : the serial COM number\n");
		printf(  "\tledcmd  : led control code, e.g., 0x30=LED#3 Off, 0x31=LED#3 ON\n");
		printf(  "\t          LED number ranges from 1 to 8\n");
		printf(  "\t          LED control: 1=ON, 0=OFF\n");
		printf("\n");
		exit(1);
	}

	int comport = atoi(argv[1]);
	int ledcmd = (int)strtol(argv[2], NULL, 16);

	printf("comport= %d, ledcmd= 0x%02X \n", comport, ledcmd);
...
//	const char * device = "\\\\.\\COM13";
	char device[32];
	snprintf(device, 31, "\\\\.\\COM%d", comport);
	printf("comport= %d --> %s\n", comport, device);
	...
	if (port == INVALID_HANDLE_VALUE) {
		printf("ERROR: can\'t open serial device - %s \n", device);
		return 1;
	}

	uint8_t u8cmd = (uint8_t)ledcmd;
	printf("Writing %s, data 0x%02X\n", device, u8cmd);
	write_port(port, &u8cmd, sizeof(u8cmd));

#if 0
	int feedback = jrk_get_feedback(port);
	if (feedback < 0) {
		printf("get feedback failed !!\n");
		return 1;
	}
	...
	int new_target = (1 < 2048) ? 2248 : 1848;
